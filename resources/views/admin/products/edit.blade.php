...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold text-primary">✏️ Edit Product</h2>
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="discount" value="{{ old('discount', $product->discount) }}" class="form-control" step="1" min="0" max="100">
                </div>
                ...
                <div class="col-md-6">
                    ...
                    @if($product->thumbnail)
                        <img src="{{ asset('storage/'.$product->thumbnail) }}" width="100" class="mt-2">
                    @endif
                </div>
                ...
                <div class="col-md-12">
					...
					<div class="mt-3 d-flex flex-wrap gap-2" id="gallery-preview">
						@foreach($product->images as $img)
						<div class="position-relative">
							<img src="{{ asset('storage/'.$img->image_path) }}" width="100" class="border rounded shadow-sm">
							<button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 delete-image" data-id="{{ $img->id }}" ><i class="fa-solid fa-xmark"></i></button>
						</div>
						@endforeach
					</div>
                </div>
...
<script>
    document.getElementById('images').addEventListener('change', function(event) {
        let preview = document.querySelector('.d-flex.flex-wrap.gap-2');
...
document.querySelectorAll('.delete-image').forEach(button => {
    button.addEventListener('click', function() {
        ...
        .then(data => {
            if(data.success) {
                parentDiv.remove(); // remove image div from DOM
            }
        })
